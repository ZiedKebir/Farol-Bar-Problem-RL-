...
class DDPGagent:
    def __init__(self,hidden_size=256, actor_learning_rate=1e-4, critic_learning_rate=1e-3, gamma=0.99, tau=1e-2, max_memory_size=50000):
        ...
        self.actor =  Actor(input_size=7, hidden_size=hidden_size, output_size=7,learning_rate = 3e-4)
        #self.actor = Actor(self.num_states, hidden_size, self.num_actions)
        self.actor_target = Actor(input_size=7, hidden_size=hidden_size, output_size=7,learning_rate = 3e-4)
        #self.actor_target = Actor(self.num_states, hidden_size, self.num_actions)

        self.critic = Critic(input_size=14, hidden_size=hidden_size, output_size=7)
       
        #self.critic = Critic(self.num_states + self.num_actions, hidden_size, self.num_actions)

        self.critic_target = Critic(input_size=14, hidden_size=hidden_size, output_size=7)
    ...
    def get_action(self, state,exploration_rate: float): ### I updated this function ### 
        if random.uniform(0,1)<=exploration_rate: #in this case the agent explores 
            return np.array([random.uniform(0,1) for i in range(0,7)])
    ...
    def update(self, batch_size):
        ...
        next_states = next_states.view(128,7)
        #print("next_states",next_states.shape)
        # Critic loss   
        #print("states",states)
        #print("actions",states)

        actions=actions.view(128, 7)
        Qvals = self.critic.forward(states, actions)
        #print("Qvals",Qvals)
        #print("Qvqls",Qvals)
        next_actions = self.actor_target.forward(next_states)
        next_actions = next_actions.view(128,7)
